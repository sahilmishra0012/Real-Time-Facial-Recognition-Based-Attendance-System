...
class FaceNetService {
  ...
  Future loadModel() async {
    try {
      ...
      print('Failed to load model.HEHEHE');
      print(e);
    }
  }
  ...
  Future<bool> predict(String userId) {
    /// search closer user prediction if exists
    return _searchResult(this._predictedData, userId);
  }
  ...
  Future<bool> _searchResult(List predictedData, userId) async {
    /// loads Firebase Firestore Collection 🙄
    CollectionReference users = FirebaseFirestore.instance.collection('Users');

    Future<DocumentSnapshot> userData = users.doc(userId).get();
    await userData.then((document) {
      faceVector = document['facial_vector'];
    });
    var lists = json.decode(faceVector);
    print(_euclideanDistance(lists, predictedData));
    if (lists.length == 0) return false;
    if (_euclideanDistance(lists, predictedData) < 1) {
      return true;
    }
    return false;
  }
